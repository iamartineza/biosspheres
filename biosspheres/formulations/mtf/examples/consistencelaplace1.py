...
def testing_mtf_linear_operators_and_matrices_one_sphere(
    big_l: int = 5, r: float = 2.1, pi: float = 3.0
) -> None:
    """
...
def testing_mtf_azimuthal_and_no_azimuthal(
    ...
    num = big_l + 1
    b = np.random.random((4 * num))
    b_2_1 = extensions.azimuthal_trace_to_general_with_zeros(big_l, b[0:num])
    ...
    solution2_3 = extensions.azimuthal_trace_to_general_with_zeros(
        big_l, solution2[2 * num : 3 * num]
    )
    solution2_4 = extensions.azimuthal_trace_to_general_with_zeros(
